...
#5
library(tibble)
newmtc <- tibble(mtcars$mpg,as.integer(mtcars$cyl),mtcars$disp,mtcars$hp,
                     mtcars$drat,mtcars$wt,mtcars$qsec,as.integer(mtcars$vs),
                     as.integer(mtcars$am),mtcars$gear,mtcars$carb)
colnames(newmtc) <- colnames(mtcars)
...
ans_10
#11
x <- as.numeric(as.character(diamonds$x))
y <- as.numeric(as.character(diamonds$y))
z <- as.numeric(as.character(diamonds$z))
diam_11 <- sum((x+y+z)>40)
ans_11=diam_11
ans_11
...
#16
newdiam <- subset(diamonds[c(1:1000),])
ans_16=newdiam
ans_16
